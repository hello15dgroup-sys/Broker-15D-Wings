    ...
    <div className="p-6 md:p-8 rounded-[2rem] border border-purple-200 shadow-xl relative overflow-hidden bg-white text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-purple-100">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          <span className="px-3.5 py-1.5 rounded-full bg-purple-100 border border-purple-200 text-purple-800 text-[10px] font-sync tracking-wider uppercase font-bold">
            100% Free Route Math
          </span>
          <div className="bg-purple-50 p-1 rounded-xl border border-purple-200 flex gap-1">
            <button
              onClick={() => setCurrency('USD')}
              className={`px-3 py-1 rounded-lg text-xs font-mono font-bold transition-all cursor-pointer ${currency === 'USD' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
            >
              USD ($)
            </button>
            <button
              onClick={() => setCurrency('NGN')}
              className={`px-3 py-1 rounded-lg text-xs font-mono font-bold transition-all cursor-pointer ${currency === 'NGN' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
            >
              NGN (₦)
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 py-6">
        ...
        <div className="space-y-2">
          <label className="font-sync text-[10px] text-gray-700 font-bold tracking-widest uppercase block">
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-white border border-purple-200 rounded-2xl px-4 py-3.5 text-sm text-gray-950 font-mono font-semibold focus:border-purple-500 outline-none transition-all cursor-pointer appearance-none shadow-sm"
            >
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-white border border-purple-200 rounded-2xl px-4 py-3.5 text-xs text-gray-950 font-semibold focus:border-purple-500 outline-none transition-all cursor-pointer appearance-none shadow-sm"
            >
      ...
      <div className="flex gap-4 pb-6">
        <button
          ...
          className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-mono font-bold transition-all border cursor-pointer ${
            tripType === 'ONE_WAY'
              ? 'bg-purple-600 border-purple-600 text-white shadow-md'
              : 'bg-purple-50/70 border-purple-200 text-purple-900 hover:bg-purple-100'
          }`}
        >
          ONE WAY FLIGHT
        </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-5 rounded-2xl bg-purple-50/60 border border-purple-200 my-2">
        <div className="space-y-1">
          <span className="text-[10px] text-gray-700 font-sync uppercase font-bold block flex items-center gap-1.5">
            <Compass className="w-3.5 h-3.5 text-purple-600" /> AIR DISTANCE
          </span>
          <span className="text-xl font-mono font-bold text-gray-950">
            {calculation.distanceNm} <span className="text-xs text-gray-600 font-normal">NM</span>
          </span>
        </div>

        <div className="space-y-1">
          <span className="text-[10px] text-gray-700 font-sync uppercase font-bold block flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-purple-600" /> FLIGHT DURATION
          </span>
          <span className="text-xl font-mono font-bold text-purple-800">
            {calculation.flightHours} <span className="text-xs text-gray-600 font-normal">HRS</span>
          </span>
        </div>

        <div className="space-y-1">
          <span className="text-[10px] text-gray-700 font-sync uppercase font-bold block flex items-center gap-1.5">
            <Fuel className="w-3.5 h-3.5 text-amber-600" /> EST. FUEL BURN
          </span>
          <span className="text-xl font-mono font-bold text-amber-700">
            {calculation.fuelGallons.toLocaleString()} <span className="text-xs text-gray-600 font-normal">GAL</span>
          </span>
        </div>

        <div className="space-y-1">
          <span className="text-[10px] text-gray-700 font-sync uppercase font-bold block flex items-center gap-1.5">
            <DollarSign className="w-3.5 h-3.5 text-emerald-600" /> BASELINE WHOLESALE
          </span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 mt-2">
        <div className="text-xs text-gray-600 font-mono">
          ...
          <span className="text-purple-800 font-bold">{origin.code} ({origin.lat}, {origin.lon})</span>
          <span> → </span>
          <span className="text-emerald-700 font-bold">{destination.code} ({destination.lat}, {destination.lon})</span>
        </div>

        <button
          onClick={handleApplyEstimate}
          className="w-full sm:w-auto px-6 py-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-sync text-xs font-bold tracking-wider uppercase transition-all shadow-md flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
        >
