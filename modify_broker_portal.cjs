...
// Check imports
if(!content.includes('import {') || !content.includes('Users')) {
  // It's importing lucide-react somewhere, let's just add the ones we need
  content = content.replace(/import \{([\s\S]*?)\} from "lucide-react";/g, 'import { $1, Users, FileText, Link, CreditCard, Radar, Database, Info } from "lucide-react";');
}
